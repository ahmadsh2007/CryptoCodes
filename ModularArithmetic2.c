...
int main(){
    ...
    printf("This computes: (base ^ exponent) mod modulus\n");

    uint64_t base, exponent, modulus;
    printf("Enter base:\t");
    if (scanf("%" SCNu64, &base) != 1) {
        printf("Invalid input for base.\n");
        return 1;
    }

    printf("Enter exponent:\t");
    if (scanf("%" SCNu64, &exponent) != 1) {
        printf("Invalid input for exponent.\n");
        return 1;
    }

    printf("Enter modulus:\t");
    if (scanf("%" SCNu64, &modulus) != 1 || modulus == 0) {
        printf("Invalid input for modulus.\n");
        return 1;
    }
    ...
    printf("\nResult: (%" PRIu64 " ^ %" PRIu64 ") mod %" PRIu64 " = %" PRIu64 "\n", base, exponent, modulus, result);
